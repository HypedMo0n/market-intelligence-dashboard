...
import { callOpenAiJson, hasOpenAiConfig } from "@/lib/providers/openai";
...
export async function POST() {
  try {
    ...
    const scan = await callOpenAiJson<Record<string, unknown>>({
      system: MACRO_SYSTEM,
    ...
    return NextResponse.json({ ...scan, fetchedAt: new Date().toISOString() });
  } catch (error) {
